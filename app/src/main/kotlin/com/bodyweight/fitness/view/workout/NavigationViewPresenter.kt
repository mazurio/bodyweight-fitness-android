...
class NavigationPresenter : AbstractPresenter() {
  ...
  fun nextExercise() {
    RoutineStream.exercise = getNextExercise()
  }

  fun getNextExercise(): Exercise {
    val bundledSection = RoutineStream.exercise.section?.bundledSection
    val bundledSectionExercise = bundledSection?.currentExercise

    val repositoryRoutine = Repository.repositoryRoutineForToday

    // no bundled Exercise for this Section
    if (bundledSectionExercise == null && RoutineStream.exercise.isNext) {
      return RoutineStream.exercise.next!!
    }

    val bundledExercise = repositoryRoutine.exercises.filter {
      it.exerciseId == bundledSectionExercise?.exerciseId
    }.firstOrNull()
    ...
    if (bundledExercise != null && bundledExercise.sets.size < bundledSection!!.sets.toInt()) {
      return bundledSectionExercise!!
    }

    if (currentExercise!!.sets.size < RoutineStream.exercise.section!!.sets.toInt()) {
